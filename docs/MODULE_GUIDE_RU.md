...
Это руководство объясняет, как создавать пользовательские модули для Telegram-бота SBS Helper.
...
### Шаг 7: Добавление кнопки главного меню (опционально)

Чтобы добавить ваш модуль в главное меню, отредактируйте `config/settings.py`:

```python
MAIN_MENU_BUTTONS: Final[List[List[str]]] = [
    ["✅ Валидация заявок", "📸 Обработать скриншот"],
    ["🔧 Мой модуль"],  # Добавьте вашу кнопку
    ["🎫 Мои инвайты", "❓ Помощь"]
]
```
