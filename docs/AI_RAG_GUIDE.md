...
- для каждой директории синхронизации разрешён только один активный процесс `rag_directory_ingest.py` (PID lock-файл в `/tmp`), повторный запуск для той же директории завершается с ошибкой,
