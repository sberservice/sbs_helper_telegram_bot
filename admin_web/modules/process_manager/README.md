...
- **Управление группами** — CRUD-интерфейс для конфигурации групп GK и The Helper (вкладка «Группы»)
...
Вкладка **«Группы»** предоставляет CRUD-интерфейс для управления конфигурацией групп, заменяя интерактивный CLI-режим `--manage-groups`.

### Секции

| Секция | Конфиг-файл | Описание |
|--------|-------------|----------|
| GK Groups | `config/gk_groups.json` | Группы для GK Collector/Responder |
| Helper Groups | `config/helper_groups.json` | Группы для The Helper |
| Собранные из БД | — | Группы из таблицы `gk_messages` (только чтение) |

### Формат конфигурации

**GK Groups** (`config/gk_groups.json`):
```json
{
  "groups": [
  ...
  "test_target_groups": [
    {"id": -1001111111111, "title": "Тестовая группа 1"},
    {"id": -1002222222222, "title": "Тестовая группа 2", "participants": 25}
  ],
  "test_target_group": {
    "id": -1001111111111,
...
`test_target_group` — активная группа для redirect test mode.
`test_target_groups` — список доступных вариантов для выбора в UI.
