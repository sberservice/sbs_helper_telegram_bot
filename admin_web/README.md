...
- **Group Knowledge** — единый модуль аналитики, валидации и тестирования Q&A из Telegram-групп:
  ...
  - *Группы* — мониторируемые Telegram-группы
    - *Автоответчик* — лог live/dry-run ответов, включая неуспешные попытки (не найден ответ / confidence ниже порога), и отдельный subpage «Обзор» со статистикой по диапазону дат
    ...
    - *Message Browser* — просмотр сырых сообщений `gk_messages` с фильтрами по группе/пользователю/дате/статусам, индикаторами chain/analyzer/question, превью изображений и сохранённого LLM-описания (`image_description`) в списке и в split-screen панели цепочки, а также полем результата обработки автоответчиком (`LIVE`/`DRY` + confidence)
