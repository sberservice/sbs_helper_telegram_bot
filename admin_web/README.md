...
- **Group Knowledge** — единый модуль аналитики, валидации и тестирования Q&A из Telegram-групп:
  - *Статистика* — обзорные метрики, тайм-лайн, распределение уверенности
    ...
    - *Экспертная валидация* — проверка Q&A-пар с горячими клавишами (`Y,Y` / `N,N` / `S`), цепочками сообщений и просмотром сохранённого LLM-запроса
        - *Тестер промптов* — слепое A/B-сравнение пользовательских промптов с Elo-рейтингом, сравнительной статистикой (глобальный рейтинг, сессии, голоса/skip), настройкой количества цепочек, предварительным расчётом числа сравнений до запуска, случайной выборкой цепочек, автоматической генерацией вариантов при создании сессии, выбором модели из динамического списка поддерживаемых моделей и применением индивидуальной `temperature` каждого промпта при генерации
  ...
  - *Автоответчик* — лог live/dry-run ответов
    ...
    - *Image Prompt Tester* — отдельный blind A/B тестер промптов описания изображений (Elo, модель, сессии, кастомные промпты, глобальная статистика, предварительный расчёт числа сравнений, случайная выборка изображений)
      - *Песочница поиска* — гибридный BM25 + Vector + RRF поиск по реальному `QASearchService` с предпросмотром итогового ответа автоответчика и отображением этапов выполнения запроса
...
- **Менеджер процессов** — компактный обзор карточек процессов для экранов с большим числом демонов
...
│   │   ├── db.py
│   │   └── router.py
│   └── gk_knowledge/        ← Модуль Group Knowledge (9 вкладок)
