...
- **Group Knowledge** — единый модуль аналитики, валидации и тестирования Q&A из Telegram-групп:
  - *Статистика* — обзорные метрики, тайм-лайн, распределение уверенности
    ...
    - *Тестер промптов* — слепое A/B-сравнение пользовательских промптов с Elo-рейтингом, настройкой количества цепочек, автоматической генерацией вариантов при создании сессии и выбором модели из динамического списка поддерживаемых моделей
  ...
  - *Автоответчик* — лог live/dry-run ответов
    - *Изображения* — очередь обработки скриншотов GigaChat Vision
        - *Песочница поиска* — гибридный BM25 + Vector + RRF поиск по реальному `QASearchService` с предпросмотром итогового ответа автоответчика
...
│   │   ├── db.py
│   │   └── router.py
│   └── gk_knowledge/        ← Модуль Group Knowledge (8 вкладок)
