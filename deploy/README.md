...
На GPU-серверах `deploy\update.bat` теперь автоматически проверяет backend `torch`
и при необходимости переустанавливает CUDA-сборку (`cu124`), чтобы не откатываться на CPU-версию.
