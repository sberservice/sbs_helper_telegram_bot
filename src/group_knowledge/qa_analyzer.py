...
9. Если ответ отвечает на ситуацию на данный момент (не работает какой-то сервис, или введено временное правило), укажи дату в ответе.
...
class QAAnalyzer:
    ...
    def _build_acronyms_section(self, group_id: int) -> str:
        ...
        Результат кэшируется с TTL = GK_TERMS_CACHE_TTL_SECONDS.
        ...
        try:
            ...
            if acronyms:
                eligible_acronyms: List[Dict[str, Any]] = []
                for a in acronyms:
                    status = str(a.get("status") or "").strip().lower()
                    expert_status = str(a.get("expert_status") or "").strip().lower()
                    if status == "rejected" or expert_status == "rejected":
                        continue

                    is_approved = status == "approved" or expert_status == "approved"
                    ...
                    if confidence_raw is not None:
                        try:
                            ...
                            confidence = None
                    if not is_approved and (confidence is None or confidence < min_confidence):
                        continue

                    term = str(a.get("term", "") or "").strip().upper()
                    definition = str(a.get("definition") or "").strip()
                    if not term:
                        continue
                    eligible_acronyms.append(a)
