...
QUESTION_CLASSIFICATION_PROMPT = """Ты определяешь, является ли сообщение вопросом для технической поддержки.

Правила:
1. Считай вопросом как явные вопрос, пусть даже без вопросительного знака.
2. Благодарности, отчёты о том, что всё работает, шутки и обычные утверждения без запроса помощи — не вопрос.
3. Если в сообщение написано, что надо предпринять какое-то действие, это НЕ вопрос.
