...
class TicketValidatorHandler(IntentHandler):
    """Обработчик тикетов для постановки задачи генерации СООС."""
    ...
    async def execute(self, params: Dict[str, Any], user_id: int) -> str:
        """Поставить текст тикета в очередь СООС для генерации изображений."""
        from src.sbs_helper_telegram_bot.soos import messages as soos_messages
        from src.sbs_helper_telegram_bot.soos import soos_parser
        ...
        if not ticket_text:
            return "⚠️ Не указан текст тикета для генерации СООС\\."

        try:
            if check_if_user_has_unprocessed_job(user_id):
                return soos_messages.MESSAGE_ACTIVE_JOB_EXISTS
            ...
            if missing_fields:
                missing_fields_text = "\n".join(
                    f"• {escape_markdown_v2(str(field))}" for field in missing_fields
                )
                return soos_messages.MESSAGE_MISSING_FIELDS.format(fields_list=missing_fields_text)
            ...
            return soos_messages.MESSAGE_TICKET_ACCEPTED.format(position=position)

        except Exception as exc:
            logger.error("Ошибка обработки тикета СООС через AI: user=%s error=%s", user_id, exc)
            return "❌ Ошибка при постановке тикета в СООС\\. Попробуйте через меню *🧾 СООС*\\."
