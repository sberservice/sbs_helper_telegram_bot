...
Если `upos_error_lookup` сработал, но код ошибки не найден в базе UPOS, роутер показывает промежуточное уведомление и автоматически делает fallback в `rag_qa` по исходному сообщению пользователя.

Для intent `certification_info` Telegram-слой автоматически переводит пользователя в меню модуля `📝 Аттестация` (сбросив зависшие состояния других модулей), если пользователь ещё не находится в этом меню.
...
- Для сценария `upos_error_lookup` с несуществующим кодом показывается отдельный этап уведомления о переходе к поиску в базе знаний, после чего запускается RAG-ответ.
