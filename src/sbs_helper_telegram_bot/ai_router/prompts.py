...
3. Если доступен intent "rag_qa" и сообщение похоже на вопрос/запрос инструкции/регламента/процедуры — ПРЕДПОЧИТАЙ "rag_qa" даже при умеренной уверенности (0.55-0.9).
...
#4. В конце добавь строку: "Источник: ..." со списком блоков, на которые опирался.
    ...
    return f"""Ты — помощник технической поддержки СберСервис.
...
7. Всегда сканируй ВСЕ фрагменты контекста на релевантность, даже если первый кажется достаточным. Собери информацию из всех подходящих фрагментов и синтезируй в coherent ответ. Если несколько фрагментов релевантны, укажи источники всех.
