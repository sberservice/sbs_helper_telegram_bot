...
def build_rag_prompt(context_blocks: List[str]) -> str:
    """
...
6. Перед анализом, обработай синонимы в запросе пользователя, заменив их на канонические термины из следующих групп (это не выдумывание фактов, а уточнение для точного поиска в контексте):
   ...
   - После замены, используй обновленный запрос для поиска по всем фрагментам контекста. Если синонимы не применены, ответь: "Ошибка: синонимы не обработаны".
