...
def build_classification_prompt(enabled_modules: List[str]) -> str:
    ...
    return f"""Ты — классификатор намерений пользователя для Telegram-бота технической поддержки.
...
8. Если пользователь вводит длинный многострочный текст с адресом — это скорее всего заявка для валидации (если модуль доступен).
9. Для intent "ticket_validation" не дублируй весь исходный текст пользователя в `parameters.ticket_text`, если он очень длинный: передавай только информативный фрагмент.
...
def build_chat_prompt() -> str:
    ...
    """
    return """Ты — дружелюбный помощник в Telegram-боте технической поддержки SBS.
