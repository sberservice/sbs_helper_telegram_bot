"""
Сообщения модуля ошибок UPOS

Все пользовательские сообщения для модуля поиска кодов ошибок UPOS.
...
def _get_errors_count() -> int:
    """
    Получить количество активных кодов ошибок из базы.

    Returns:
        Число активных кодов ошибок.
    """
    try:
...
def format_error_code_response(
    ...
    """
    Отформатировать информацию о коде ошибки для показа пользователю.

    Args:
        error_code: Код ошибки.
        description: Описание ошибки.
        suggested_actions: Рекомендации по устранению.
        category_name: Название категории (опционально).
        updated_timestamp: Unix-время последнего обновления (опционально).

    Returns:
        Отформатированное сообщение MarkdownV2.
    """
...
def format_error_list_item(
    ...
    """
    Отформатировать код ошибки для списка (админ).

    Args:
        error_code: Код ошибки.
        description: Короткое описание (будет обрезано).
        category_name: Категория (опционально).
        times_requested: Количество запросов (для популярных).
...
def format_unknown_code_item(error_code: str, times_requested: int, last_timestamp: int) -> str:
    """
    Отформатировать неизвестный код для списка.

    Args:
        error_code: Неизвестный код ошибки.
        times_requested: Количество запросов.
        last_timestamp: Время последнего запроса.
...
def format_category_list_item(name: str, error_count: int, display_order: int = 0) -> str:
    """
    Отформатировать категорию для списка.

    Args:
        name: Название категории.
        error_count: Количество ошибок в категории.
        display_order: Порядок отображения (не используется, оставлено для совместимости API).

    Returns:
        Отформатированная строка списка.
    """
    _ = display_order  # Не используется, оставлено для совместимости API
