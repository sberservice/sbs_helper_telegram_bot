...
• *regex\\_not\\_match* \\- регулярное выражение \\(НЕ должно совпасть\\)
• *regex\\_fullmatch* \\- регулярное выражение \\(полное совпадение\\)
• *regex\\_not\\_fullmatch* \\- регулярное выражение \\(НЕ полное совпадение\\)
• *required\\_field* \\- обязательное поле
• *format* \\- формат \\(phone, email, date, inn\\)
• *length* \\- проверка длины \\(min:X,max:Y\\)
• *custom* \\- пользовательское
• *fias* \\- проверка адреса через ФИАС"""

MESSAGE_ADMIN_CREATE_RULE_PATTERN = r"""Введите *паттерн* \(регулярное выражение или спецификацию\)\.

*Примеры:*
• regex: `ИНН[:\s]*\\d{10,12}`
• format: `phone` или `date`
• length: `min:10,max:1000`
• fias: regex для извлечения адреса \(первый capture group\), пустое значение использует стандартный шаблон"""
