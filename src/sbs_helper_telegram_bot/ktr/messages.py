...
MESSAGE_KTR_LAST_UPDATE_UNKNOWN = "не указана"

MESSAGE_KTR_FEEDBACK_HINT = (
    "ℹ️ Если у вас есть более свежий файл КТР, через модуль "
    "«📬 Обратная связь» укажите, как с вами связаться\\."
)
...
MESSAGE_ADMIN_SET_LAST_UPDATE_DATE = (
    "📅 *Дата последнего обновления КТР*\n\n"
    "Текущее значение: *{current_date}*\n\n"
    "Введите дату в формате *дд\\.мм\\.гггг*\\.\n"
    "Чтобы очистить значение, отправьте `\\-`\\."
)

MESSAGE_ADMIN_INVALID_LAST_UPDATE_DATE = (
    "⚠️ Некорректная дата\\. Используйте формат *дд\\.мм\\.гггг* "
    "или отправьте `\\-`, чтобы очистить значение\\."
)

MESSAGE_ADMIN_LAST_UPDATE_DATE_SAVED = "✅ Дата последнего обновления КТР сохранена: *{date}*"
MESSAGE_ADMIN_LAST_UPDATE_DATE_CLEARED = "✅ Дата последнего обновления КТР очищена\\."
...
def get_entry_message(last_update_date: Optional[str] = None) -> str:
    """
    Сформировать стартовое сообщение модуля КТР с датой последнего обновления.

    Args:
        last_update_date: Дата последнего обновления общего файла КТР (дд.мм.гггг).

    Returns:
        Сообщение в формате MarkdownV2.
    """
    safe_date = escape_markdown_v2(last_update_date or MESSAGE_KTR_LAST_UPDATE_UNKNOWN)

    return (
        "⏱️ *КТР \\(Коэффициент Трудозатрат\\)*\n\n"
        f"📅 *Последнее обновление кодов:* *{safe_date}*\n\n"
        f"{MESSAGE_KTR_FEEDBACK_HINT}\n\n"
        f"{MESSAGE_ENTER_CODE}"
    )
