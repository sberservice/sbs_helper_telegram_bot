...
- **🔢 UPOS Ошибки** — поиск кодов ошибок UPOS с описаниями и рекомендациями по устранению
- **⏱️ КТР** — поиск кодов трудозатрат с указанием нормативного времени выполнения
- **📝 Сертификация** — тестирование сотрудников с рейтингами и историей попыток
...
- **Поиск кодов ошибок**: Быстрый поиск с описаниями и рекомендациями по устранению
- **Категоризация**: Ошибки организованы по типам 
- **Статистика запросов**: Топ популярных ошибок и отслеживание неизвестных кодов
- **Импорт из CSV**: Массовая загрузка кодов ошибок для администраторов
- **Админ-панель**: Управление кодами ошибок и категориями
...
- **Поиск кодов КТР**: Быстрый поиск кодов с указанием нормативного времени выполнения
- **Категоризация**: Коды организованы по типам работ
- **Популярные коды**: Просмотр топа запрашиваемых кодов
- **Статистика запросов**: Отслеживание неизвестных кодов
- **Импорт из CSV**: Массовая загрузка кодов для администраторов
...
- **Перемешивание вариантов**: Порядок ответов меняется для каждого вопроса
- **Категории вопросов**: Тестирование по конкретным категориям или всем сразу
- **Множественный выбор**: 4 варианта ответа на каждый вопрос
- **Ежемесячный рейтинг**: ТОП-10 лучших результатов за месяц
- **История попыток**: Полная история прохождения тестов
- **Контроль актуальности**: Автоматическое отслеживание устаревших вопросов
- **Настройки**: Показ правильных ответов, анонимизация рейтинга
...
│   ├── upos_error/            # Модуль UPOS ошибок
│   │   ├── upos_error_bot_part.py
│   │   └── *.md               # Документация
│   └── certification/         # Модуль сертификации
│       ├── certification_logic.py # Бизнес-логика
│       ├── certification_bot_part.py
│       ├── admin_panel_bot_part.py
│       └── *.md               # Документация
config/
...
- [Тестовые шаблоны](src/sbs_helper_telegram_bot/ticket_validator/TEST_TEMPLATES.md)
- [UPOS Ошибки](src/sbs_helper_telegram_bot/upos_error/README.md)
- [КТР (Коэффициент Трудозатрат)](src/sbs_helper_telegram_bot/ktr/README.md)
- [Сертификация сотрудников](src/sbs_helper_telegram_bot/certification/README.md)
